...
def load_survey_data():
    ...
    if os.path.exists(SURVEY_DATA_FILE):
        ...
        # Automatically upgrade older CSV files with missing columns if they exist
    ...
    with st.expander("❌ De-register or Purge Erroneous Record"):
        with st.form('registry_removal_form'):
            if not current_df.empty:
                ...
                # Only try to grab the row data if matching_rows is NOT empty
                if remove_submitted and not matching_rows.empty:
                    ...
                    st.error("Could not find the selected record. It may have already been deleted.")
...
if not current_df.empty:
    ...
    # A score from 0 to 15 showing danger level (Higher score means smaller MUAC, i.e., severe thinness)
